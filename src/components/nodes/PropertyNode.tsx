        ...
        <Collapsible className="flex flex-col gap-4" defaultOpen>
            <CollapsibleTrigger className="group flex place-content-between place-items-center rounded-md p-2 hover:bg-[#e7e7e9] dark:hover:bg-[#242428]">
                <h2 className="flex place-items-center gap-2 text-xl font-bold">
                    <VscSymbolProperty aria-hidden className="shrink-0" size={24} />
                    Properties
                </h2>
            ...
            <CollapsibleContent>
                <div className="flex flex-col gap-4">
                    {node.map((property, idx) => (
                        <Fragment key={`${property.displayName}-${idx}`}>
                            <div className="flex flex-col gap-4">
                                <div className="flex place-content-between place-items-center gap-1">
                                    <h3
                                        className="scroll-mt-8 group flex flex-col gap-2 px-2 font-mono font-semibold break-all"
                                    ...
                                    >
                                        ...
                                        <span>
                                            ...
                                            <span className={resolveNodeKindSpanClass(property.kind)}>
                                                {property.displayName}
                                            </span>
                                            {property.isOptional ? "?" : ""} :{" "}
                                            <ExcerptNode node={property.typeExcerpt} version={version} />{" "}
                                            {property.summary?.defaultValueBlock.length
                                                ? `= ${property.summary.defaultValueBlock.reduce(
                                                      (acc, def) => `${acc}${def.text}`,
                                                      "",
                                                  )}`
                                                : ""}
                                        </span>
                                    ...
                                    <Link
                                        ...
                                        className="min-w-min"
                                        href={
                                            property.sourceLine !== -1
                                                ? `${property.sourceURL}#L${property.sourceLine}`
                                                : property.sourceURL
                                        }
                                ...
                                {property.summary?.deprecatedBlock.length ? (
                                    <DeprecatedNode
                                        deprecatedBlock={property.summary.deprecatedBlock}
                                        version={version}
                                    />
                                ) : null}
