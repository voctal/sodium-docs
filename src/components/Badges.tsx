...
export function Badge({ children, className }: PropsWithChildren<{ readonly className?: string }>) {
    return (
        <span
            className={cn(
                "inline-flex place-items-center gap-1 rounded-full px-2 py-1 font-sans text-sm leading-none font-normal whitespace-nowrap",
                className,
            )}
        >
            {children}
...
export async function Badges({ node }: { readonly node: Partial<Node> }) {
