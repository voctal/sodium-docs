        ...
        searchResults?.map((item, idx) => (
            ...
            >
                ...
                <div className="flex grow flex-col">
                    <span className={cn("font-semibold wrap-anywhere", resolveNodeKindSpanClass(item.kind))}>
                        {item.name}
                    </span>
                    {/* <span className={cn("truncate text-sm", isMobile ? "max-w-[30ch]" : "max-w-[40ch]")}> */}
                    {/* {item.summary} */}
                    {/* </span> */}
                    <span className={cn("truncate text-xs", isMobile ? "max-w-[30ch]" : "max-w-[40ch]")}>
                        {item.path}
                    </span>
                </div>
    ...
    useEffect(() => {
        ...
        const searchDoc = async (searchString: string, version: string) => {
            const res = await fetch(
                `/api/docs/search?packageName=${encodeURIComponent(packageName)}&version=${version}&q=${encodeURIComponent(searchString)}`,
            );
        ...
        if (search && packageName) {
            void searchDoc(search, branchName);
        } else {
            setSearchResults([]);
        }
